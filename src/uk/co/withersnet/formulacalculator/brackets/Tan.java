...
public class Tan extends OpenBracket {
	...
	public Number modify(Number n) {
		...
		if(GlobalConstants.deg){
			return new Number(df.format(Math.tan(Math.toRadians(n.valueOf()))));
		}else{
			return new Number(df.format(Math.tan(n.valueOf())));
		}
	}
	
	@Override
	public void updateBounds(int x, int y, Paint paint) {
		paint.getTextBounds("tan(", 0, 3, bounds);
